...
import CachePolicy from '../vendor/http-cache-semantics';
import { bgGreen, black, cyan, dim, green } from 'kleur/colors';
import fs from 'node:fs/promises';
import OS from 'node:os';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import type { SSRImageService, TransformOptions } from '../loaders/index';
import { debug, info, LoggerLevel, warn } from '../utils/logger';
...
function webToCachePolicyRequest({ url, method, headers: _headers }: Request): any {
	const headers: any = {};
...
function webToCachePolicyResponse({ status, headers: _headers }: Response): any {
	const headers: any = {};
	for (const [key, value] of _headers) {
		headers[key] = value;
	}
